# CLAUDE.md — Contrato operacional para Claude sob protocolo ESAA
...
Este documento é o contrato operacional para agentes como Claude neste repositório.
Ele deve ter a mesma cobertura normativa de `AGENTS.md`, com adaptações
explícitas para o modo de trabalho do Claude.
...
- Em execução governada, você nunca modifica arquivos do projeto diretamente com ferramentas do Claude (Edit, Write, Bash etc.). Artefatos produzidos vão em `file_updates` dentro do envelope, e o Orchestrator os aplica.
...
- não use ferramentas do Claude (Edit, Write, Bash etc.) para editar arquivos finais diretamente;
...
- não aplique diretamente os arquivos com ferramentas do Claude.
...
só com `complete`, output governado é JSON puro, Claude não aplica efeitos diretamente,
